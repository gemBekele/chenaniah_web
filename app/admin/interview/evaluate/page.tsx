...
export default function InterviewEvaluationPage() {
  ...
  const getToken = () => {
    // Try judge token first, then admin token
    let token = localStorage.getItem("judge_token") || sessionStorage.getItem("judge_token")
    if (!token) {
  ...
  const fetchVerifiedAppointments = async () => {
    setIsLoading(true)
    try {
      // Use the new endpoint for judges (only present and approved)
  ...
  useEffect(() => {
    ...
    // Check if judge token exists, otherwise check admin
    const judgeToken = localStorage.getItem("judge_token") || sessionStorage.getItem("judge_token")
    const adminToken = localStorage.getItem("admin_token") || sessionStorage.getItem("admin_token")
    
    if (!judgeToken && !adminToken) {
      // Redirect to appropriate login
      if (window.location.pathname.startsWith('/judge')) {
        router.push("/judge")
      } else {
        router.push("/admin")
      }
      ...
      <div className="container mx-auto px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Evaluate verified applicants on various criteria. Ratings are on a scale of 0-5.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <Card className="p-6">
              <h2 className="text-xl font-semibold mb-4">Verified Applicants</h2>
              <div className="space-y-2">
                {appointments.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No verified applicants yet</p>
                ) : (
