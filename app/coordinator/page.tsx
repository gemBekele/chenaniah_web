...
export default function CoordinatorLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/auth/coordinator/login`, {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-muted/30 via-background to-muted/20">
      <Card className="w-full max-w-md p-8 shadow-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2">Coordinator Login</h1>
          <p className="text-muted-foreground">Sign in to manage interview attendance</p>
        </div>
